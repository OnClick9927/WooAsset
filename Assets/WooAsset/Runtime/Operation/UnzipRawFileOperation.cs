...
namespace WooAsset
{
    public class UnzipRawFileOperation : Operation
    {
        ...
        {
            _progress = 0f;
            if (!AssetsInternal.Initialized())
            {
                SetErr("InitAsync Filrst ");
            }
            else
            {
                ...
                {
                    string path = files[i];
                    var asset = await AssetsInternal.LoadAsset(path, true,typeof(RawObject)) as Asset;
                    if (asset.isErr)
                    {
                        SetErr(asset.error);
                        break;
